    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        <select
          ...
          className="bg-[#0c1628] border border-white/10 rounded-md px-2 py-2 text-xs outline-none focus:border-zen-accent"
        >
            ...
            {pageRows.map((t) => (
              <tr key={t.hash} className="border-b border-white/5 hover:bg-white/5">
                ...
                <td className="py-2 pr-3 whitespace-nowrap">
                  ...
                  <span className={cls(
                    "ml-2 text-[10px] px-2 py-[2px] rounded-full",
                ...
                <td className="py-2 pr-3">
                  {t.from ? <a href={explorerAddressUrl(t.from)} target="_blank" className="hover:underline">{shortHash(t.from)}</a> : "—"}
                </td>
                <td className="py-2 pr-3">
      ...
      <div className="flex items-center gap-2">
        <button className="px-3 py-1.5 rounded-md bg-white/10 hover:bg-white/20 text-xs" onClick={() => setPage(p => Math.max(1, p-1))} disabled={page <= 1}>
          Prev
        </button>
        <div className="text-xs text-zen-sub">Page {page} / {totalPages}</div>
        <button className="px-3 py-1.5 rounded-md bg-white/10 hover:bg-white/20 text-xs" onClick={() => setPage(p => Math.min(totalPages, p+1))} disabled={page >= totalPages}>
          Next
        </button>
      </div>
    </div>
